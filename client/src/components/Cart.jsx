...
import { FcClearFilters } from '../assets/icons'
...
const Cart = () => {
    ...
    return (
        <motion.div {...slideIn} className='fixed z-50 top-0 right-0 w-300 md:w-508 bg-cardOverlay backdrop-blur-md shadow-md h-screen'>
            ...
            <div className='flex-1 flex flex-col h-full items-start justify-start rounded-t-3xl bg-zinc-900 py-6 gap-3 relative'>
                {cart && cart?.length > 0 ? (
                    <>
                        ...
                        <div className='bg-zinc-800 rounded-t-[60px] w-full h-[70%] flex flex-col items-center justify-center px-4 py-6 gap-24'>
...
export const CartItemCard = ({ index, data }) => {
    ...
    const decrementCart = (productId) => {
        dispatch(alertSuccess("Updated the cart items"))
        increaseItemQuantity(user?.user_id,productId,"decreament").then((data)=>{
    ...
    return (
        <motion.div key={index} {...staggerFadeInOut(index)}
            className='w-full flex items-center justify-start bg-zinc-700 rounded-md drop-shadow-md px-4 gap-4'
        >
            <img
                src={data?.imageURL}
            ...
            <div className='flex items-center justify-start gap-1 w-full'>
                ...
                <p className='text-lg font-semibold text-red-400 ml-auto'>
                    ₹{itemTotal}
                </p>
            </div>
